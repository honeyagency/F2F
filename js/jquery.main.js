...
function initCarouselPartner() {
	...
	ResponsiveHelper.addRange({
		...
		'501..': {
			on: function() {
				while (carouselSlideset.children('div:not(.slide)').length)
				carouselSlideset.children('div:not(.slide):lt(2)').wrapAll('<div class="slide">');
				clearTimeout(resizeTimer);
				resizeTimer = setTimeout(function() {
...
;(function($) {
	...
	MobileDropdown.prototype = {
		...
		showSlide: function(item, drop) {
			...
			this.holder.css({ height: drop.outerHeight() + this.options.extraHeight });
	...
	$.fn.mobileDropdown = function(opt) {
		return this.each(function() {
			jQuery(this).data('MobileDropdown', new MobileDropdown($.extend(opt, { holder: this })));
		});
	};
}(jQuery));
